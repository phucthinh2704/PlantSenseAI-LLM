...
async def create(role_data: RoleCreate, db: Session = Depends(get_db)):
    result = role_service.create(role_data, db)
    return APIResponse(success=True, message="Tạo vai trò thành công", data=result)
...
async def get_all(db: Session = Depends(get_db)):
    ...
    return APIResponse(
        success=True, message="Lấy danh sách vai trò thành công", data=result
    )
...
async def detail(role_id: int, db: Session = Depends(get_db)):
    ...
    return APIResponse(
        success=True, message="Lấy chi tiết vai trò thành công", data=result
    )


@router.put("/update/{role_id}", response_model=APIResponse[RoleResponse])
async def update(role_id: int, role_data: RoleUpdate, db: Session = Depends(get_db)):
    result = role_service.update(role_id, role_data, db)
    return APIResponse(success=True, message="Cập nhật vai trò thành công", data=result)


@router.delete("/delete/{role_id}", response_model=APIResponse)
async def delete(role_id, db: Session = Depends(get_db)):
