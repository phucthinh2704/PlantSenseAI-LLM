...
async def create_disease(disease: Disease):
    inserted_id = await disease_service.create_disease(disease.model_dump(by_alias=True))
    return APIResponse(success=True, message="Disease created", data={"inserted_id": inserted_id})


@router.get("/", response_model=APIResponse)
async def get_diseases(plant_id: str | None = Query(None)):
    diseases = (
        await disease_service.get_diseases_by_plant(plant_id)
        if plant_id else await disease_service.get_all_diseases()
    )
